-- Integrated from InvisibleInc r282332 9/26/2018.
--
...
local function overridePlurality( str )
    ...
    if str then
        ...
        if not fn then
            ...
            print( "LOC: Plurality function:", str )
            loc.convertPlurality = fn
        end
    end
end
...
function Translator:LoadPOFile( filepath,lang)
	print( "Translator:LoadPOFile - loading file: "..filepath )
	local file = io.open( filepath )
	if not file then
		printf( "Translator:LoadPOFile - Language file '%s' not found.", filepath )
		return false
	end
	...
	for line in file:lines() do
		...
		if not current_id then
			...
			if c2 then
				current_id = c2
				--print( "\tFound new format id: "..tostring(c2) )
            else
		...
		elseif msgstr_flag then
			...
			if not c2 then
				...
				print( "\tFound id: "..current_id.."\tFound str: "..current_str )
		...
		elseif current_id then
			...
			elseif c2 then
				...
				printf( "LOG_LOC", "\tFound id: %s\t\t\t%s", tostring(current_id), tostring(c2))
				current_id = false
			end
		else
			--skip line
	...
	printf( "Translator:LoadPOFile( '%s' ) -- Done!", filepath )
	return true
end
...
function Translator:GetTranslatedString(strid, lang)
	...
	printf( "\tReqested id: '%s' => '%s'", strid, tostring(self.languages[lang][strid]) )
	...
	for k,v in pairs(tbl) do
		...
		if type(v) == "table" then
			...
			if str then
				...
				printf( "Translation missing: %s = \"%s\"", path, v)
			end
		end
	end
end
...
local function translateStringTable( root, tbl, fname, lang )
    ...
    if ok then
        ...
        print( "\tReverting to default language." )
        return false
	end
end
...
local function AggregateEntries( file, tbl_lookup )
	...
	for path, msgid  in pairs(tbl_lookup) do
        ...
        if warning then
            printf( "WARNING '%s': [%d] in '%s'", tostring(warning), #msgid, msgid )
        end
        if ok then
            local lines = {}
            ...
            printf( "NOT EXPORTED: [%s] = \"%s\"", path, str )
		end
	end
	...
	for k,v in pairs(tbl) do
		...
		elseif type(v) == "string" then
			if not tbl_lookup[path] then
	            ...
	            printf( "WARNING duplicate string path?? [%s] = \"%s\", new val \"%s\"", path, tostring(tbl_lookup[path]), v )
			end
			count = count + 1
		end
	end
...
local function generatePot( )
    ...
    print( "Exported to data/"..filename )
end	
